    ...
    const fetchOrders = async () => {
      try {
        ...
        const myParties = allParties.filter(party => 
            // Verifica se a festa tem dono e se é o usuário atual (pelo username ou id)
        ...
        const past = myParties.filter(p => p.status === 'REJECTED');
      ...
      <div>
        ...
        {activeOrders.length === 0 ? (
            ...
            {activeOrders.map((order) => (
                <div
                key={order.id}
                className={`castello-card p-6 ${getStatusStyle(order.status)}`}
                >
      ...
      <div>
        ...
        {pastOrders.length === 0 ? (
            ...
            {pastOrders.map((order) => (
                <div key={order.id} className="castello-card p-6 border border-gray-300 bg-gray-50 opacity-75">
